...
lint: ## check style with ruff
	uv run ruff check datakit_gitlab tests
...
coverage: ## check code coverage quickly with the default Python
	uv run coverage run --source datakit_gitlab -m pytest
	uv run coverage report -m
	uv run coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/datakit_gitlab.rst
	rm -f docs/modules.rst
	uv run sphinx-apidoc -o docs/ datakit_gitlab
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
_bump:
	...
	uv lock && \
	git add setup.cfg pyproject.toml datakit_gitlab/__init__.py uv.lock && \
